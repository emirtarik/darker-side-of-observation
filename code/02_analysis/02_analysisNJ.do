...
$c_OKeep $c_OGive -- keep give distinction
*/
...
global user "/Users/nicolasjacquemet/Dropbox" // NJ
...
* NJ
cd "$user/Soself/ExpeData/CLEAN/"
global path_fig "./results/2024.05.Slides/"
global path_tab "./CLEAN/results/tables/"
global path_raw_data "./data/raw/" 
global path_analysis_data "./data/analysis/" 

	set scheme s1color
		// Donations 
	global c_D  gs10
	global c_Dp olive_teal
	global c_Da eltgreen
	
		// Evaluations
	global c_OKeep eltblue // sienna 
	global c_OGive emidblue // stone
	
		// Perceived evaluations --- could be sienna stone
	global c_evalDp eltblue
	global c_evalDa emidblue
{
	/*********
	* Set up *
	**********/
...
label define choice_judged_otherlabel 1 "Judge decision to Give" 0 "Judge decision to Keep"
...
label define donationlabel 1 "Decided to Give" 0 "Decided to Keep"
...
label define pred_donationlabel 1 "Predicted to Give" 0 "Predicted to Keep"
...
/* Evaluations */
	...
	cibar evaluation if info==0 & inlist(treatment, 4, 6), over1(treatment) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean evaluation") title(""))
	graph export "${path_fig}/NoInfoEvals.pdf", replace

	cibar evaluation if treatment==6 & info==0, over2(donation) over1(choice_judged_other) barcol($c_OKeep $c_OGive) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean evaluation") title(""))	
	graph export "${path_fig}/EvalsOa.pdf", replace
	
	cibar evaluation if treatment==6 & info==0, over1(donation) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean evaluation") title(""))
	graph export "${path_fig}actual_b_l.pdf", replace
	
	cibar evaluation if treatment==4 & info==0, over1(LRpred_donation) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean evaluation") title("")) 
	graph export "${path_fig}predicted_b_1.pdf", replace 	
...
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean expected evaluation"))
 graph export  "${path_fig}ExpectedEvals.pdf", replace   

gen expected_keep = (expected_proportion_don <50)
cibar expected_evaluation if treatment<4 , over2(expected_keep) over1(donation) over3(treatment) barcol(eltblue emidblue) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean expected evaluation"))


 cibar expected_evaluation if treatment<4 & expected_proportion_don <.5 , over1(treatment) over2(donation) barcol(eltblue emidblue) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean expected evaluation"))
...
generate OaObskeep = obskeep * dummy_oa

gen selfish = 1 if treatment==4 & donation == 0
tab selfish treatment if info==0, nola

eststo clear
global CONTROLS $pre_treatment_vars

reg evaluation dummy_oa obskeep OaObskeep $CONTROLS if info == 0 
eststo model0

replace selfish = 1 if treatment==4
reg evaluation dummy_oa obskeep OaObskeep $CONTROLS if info == 0 & selfish == 1
eststo model1

replace selfish = 0 if treatment == 4 
reg evaluation dummy_oa obskeep OaObskeep $CONTROLS if info == 0 & selfish == 0
eststo model2

esttab model0 model1 model2 using "${path_fig}reg1a.tex" ///
	, replace ar2 label ///
    keep(dummy_oa obskeep OaObskeep) ///
	main(varlabels("Evaluation" "Dummy \$O_a$" "Obskeep" "Dummy \$O_a$ * Obskeep")) ///
    cells(b(fmt(3) star) se(fmt(3) par)) ///
    title("Regression Results") ///
	mtitles("Overall" "Oa Keeps" "Oa Gives") ///
...
cibar donation if treatment!=4, over1(treatment) over2(info) barcol($c_Dp $c_Da $c_D)  bargap(10) ///
...
cibar donation if treatment!=4 & info==0, over1(treatment) barcol($c_Dp $c_Da $c_D)  bargap(10) ///
graphopts(graphregion(fcolor(white)) ///
ysc(range(0 0.4)) ylabel(0 (0.05) 0.4) legend(cols(1) pos(5) ring(0) region(lcolor(black))) ytitle("Mean donation"))
...
cibar evaluation if treatment==4 & info==0, over1(LLpred_donation) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
...
* with INFO
cibar evaluation if treatment==6 & info==1, over1(donation) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean evaluation") title("Active observers: real share"))
 graph save "${path_fig}actual_a_2.gph", replace
 
cibar evaluation if treatment==4 & info==1, over1(LLpred_donation) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean evaluation") title("Passive observers: LL pred share")) 
...
cibar evaluation if treatment==4 & info==1, over1(LRpred_donation) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean evaluation") title("Passive observers: LR pred share")) 
...
cibar evaluation if treatment==4 & info==0, over1(LPMpred_donation) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
...
cibar evaluation if treatment==4 & info==1, over1(LPMpred_donation) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
graphopts(graphregion(fcolor(white)) ysc(range(2 5)) ylabel(2 (1) 5) legend(cols(1) pos(11) ring(0) region(lcolor(black))) ytitle("Mean evaluation") title("Passive observers: LPM pred share")) 
...
cibar evaluation if treatment==4 & info==0, over1(Gpred_donation) over2(choice_judged_other) barcol($c_OKeep $c_OGive) ///
...
cibar asso_shown_eval if treatment==6, over1(donation)  barcol($c_OKeep $c_OGive)  bargap(10) ///
...
cibar asso_shown_eval if treatment==6, over1(donation) over2(asso_shown) barcol($c_OKeep $c_OGive)  bargap(10) ///
...
cibar donation if treatment!=4, over1(treatment) over2(info) barcol($c_D $c_Dp $c_Da)  bargap(10) ///
graphopts(graphregion(fcolor(white)) ///
ysc(range(0 0.4)) ylabel(0 (0.05) 0.4) legend(cols(1) pos(5) ring(0) region(lcolor(black))) ytitle("Mean donation"))
	graph export  "${path_fig}fig4_byinfo.pdf", replace
...
cibar donation if treatment!=4 & info==0, over1(treatment) barcol($c_D $c_Dp $c_Da)  bargap(10) ///
graphopts(graphregion(fcolor(white)) ///
ysc(range(0 0.4)) ylabel(0 (0.05) 0.4) legend(cols(1) pos(5) ring(0) region(lcolor(black))) ytitle("Mean donation"))
	graph export  "${path_fig}fig4.pdf", replace	
